...
def render_html(s: Stats) -> str:
    ...
    return (
        ...
        "<div class='section'><h2>Конверсии (Метрика)</h2>"
        f"<p class='muted'>Всего: {s.conv_total} · выгружено {s.conv_uploaded} · в очереди {s.conv_pending}</p>"
        f"<div class='chips'>{_chips(s.conv_by_target)}</div></div>"
