...
3. В «Переменные и секреты» Amvera (НЕ в git) задать:
   ...
   - на этап 7 (сквозная аналитика): `GA4_MEASUREMENT_ID`, `GA4_API_SECRET`, `YM_COUNTER_ID`, `YM_OAUTH_TOKEN`, `YM_UPLOAD_INTERVAL`, `SITE_ORIGIN`
...
Связывает веб-визит на лендинге с действиями пользователя в боте через короткий одноразовый токен.

### Как работает

1. **Лендинг → токен.** При клике на кнопку «Открыть бота» лендинг отправляет `POST /api/token` с GA4 `client_id`, Метрика `client_id` и UTM-параметрами. Бот возвращает 12-символьный hex-токен.
2. **Открытие бота.** Лендинг редиректит пользователя на `t.me/bot?start=tok_<token>`. Если API не ответил за 1.2 с — открывается legacy-ссылка `start=<src>__<ym_cid>`.
3. **`/start` — склейка.** Хендлер разбирает параметр:
   - `tok_<id>` — ищет токен в БД (`tokens`), копирует `ga4_cid`/`ym_cid`/UTM на пользователя.
   - `<src>__<cid>` — legacy-формат: `utm_source` + Метрика `client_id`.
   - `<src>` — только источник (голый UTM).
4. **GA4 Measurement Protocol.** При `bot_start` / `lead_submit` / `promo_issue` бот шлёт серверное событие в GA4 (`/mp/collect`), если у пользователя сохранён `ga4_cid` и задан `GA4_API_SECRET`.
5. **Яндекс.Метрика (оффлайн-конверсии).** На `lead_submit` / `promo_issue` запись с `ym_cid` помещается в таблицу `conversions`. Фоновая задача каждые `YM_UPLOAD_INTERVAL` секунд батчем выгружает необработанные конверсии через API Метрики (`/offline_conversions/upload`) и помечает их `uploaded = true`.

### Переменные ENV

| Переменная           | Дефолт                  | Описание                                                          |
|----------------------|-------------------------|-------------------------------------------------------------------|
| `GA4_MEASUREMENT_ID` | `G-6XY6XFGE6H`          | Идентификатор потока GA4 (публичный)                              |
| `GA4_API_SECRET`     | _(пусто)_               | Секрет Measurement Protocol. Без него GA4-события не шлются       |
| `YM_COUNTER_ID`      | `110157768`              | Номер счётчика Яндекс.Метрики (публичный)                         |
| `YM_OAUTH_TOKEN`     | _(пусто)_               | OAuth-токен Метрики. Без него выгрузка конверсий не запускается   |
| `YM_UPLOAD_INTERVAL` | `1800`                  | Интервал авто-выгрузки конверсий в секундах                       |
| `SITE_ORIGIN`        | `https://russianlineup.ru` | Разрешённый CORS-origin для `POST /api/token`                  |

Все внешние вызовы (GA4, Метрика) — **best-effort**: ошибка не ломает флоу бота. Переменные `GA4_API_SECRET` и `YM_OAUTH_TOKEN` — **секреты**, задавать только через ENV/переменные Amvera, не в коде.

### Таблицы БД

- **`tokens`** — хранит токены с `ga4_cid`, `ym_cid`, UTM и `user_id` после склейки.
- **`conversions`** — очередь конверсий для выгрузки в Метрику: `id`, `ts`, `user_id`, `ym_cid`, `target`, `price`, `uploaded`.
- Таблица `users` расширена колонками `ga4_cid`, `ym_cid`.
